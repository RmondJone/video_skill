...
| 阶段 | 状态输出 | 进度条 |
|------|----------|--------|
| 1️⃣ 验证环境依赖 | "正在验证 FFmpeg 和 PySceneDetect..." | 无（快速检查） |
| 2️⃣ 获取视频信息 | "正在获取视频信息..." | 无（单次检查） |
| 3️⃣ 场景检测 | "正在检测视频场景..." | PySceneDetect 内部进度 |
| 4️⃣ 关键帧提取 | "正在提取关键帧 [██░░░░░] 3/10" | ✅ tqdm 进度条 |
| 5️⃣ 画面分析 | "正在分析关键帧图片..." | AI 处理，无进度条 |
| 6️⃣ 解说文案生成 | "正在生成解说文案 [██████░░░░] 180/300" | ✅ tqdm 进度条 |
| 7️⃣ 导出文件 | "正在保存文件..." | 无（快速保存） |

### 增量执行说明

**【重要特性】** 主流程脚本支持增量执行：

- 如果输出目录中已存在 `scenes.json`，自动跳过场景检测步骤
- 如果输出目录中已存在 `keyframes/keyframes.json`，自动跳过关键帧提取步骤

这意味着：
- 首次执行：完成完整流程（场景检测 → 关键帧 → 描述生成 → 解说生成）
- 后续执行（文件已存在）：直接跳过已完成的步骤，节省大量时间
...
**【增量执行】** 如果 `output/scenes.json` 已存在，自动跳过此步骤。
...
**【增量执行】** 如果 `output/keyframes/keyframes.json` 已存在，自动跳过此步骤。
...
⚠️ **【重要】每个场景的 description 必须包含足够多的短句！**

**数量要求**：
- 每个场景描述必须包含 **15-20 个短句**
- 短句用中文标点（，。！？；）分割
- 总句子数量 = 场景数 × 15-20

**计算公式**：
- 所需字幕数 = 视频时长(秒) / 每句时长(秒)
- 每个 description 需提供 ~所需字幕数/场景数 个短句

**示例**：10分钟视频，2秒/句，10个场景
- 所需字幕总数：600/2 = 300 句
- 每场景应提供：300/10 = 30 个短句

**禁止**：
- ❌ description 只写 2-3 个短句（会导致重复）
- ❌ 简单概括场景而不详细描述
